...
module.exports.execute = async(client, message, args,ayar,emoji) => {
   // if(!message.member.roles.cache.array().some(rol => message.guild.roles.cache.get(ayar.staffrole).rawPosition <= rol.rawPosition)) return  message.reply("`Bu komut yetkililere özeldir.`");
    ...
    MemberStats.findOne({ guildID: message.guild.id, userID: uye.id }, (err, data) => {
        ...
        embed.addField('**Bu Yıl Aktifliğin**',`\`Genel Toplam Ses =>\` ** ${client.convertDuration(data.totalVoiceStats || 0)}**\n\`Genel Toplam Chat =>\` ** ${data.totalChatStats || 0} mesaj**`);
        embed.addField('Haftalık Ses Aktifliğin',`\`Toplam =>\`  ** ${client.convertDuration(haftalikSesToplam)}** \n ${haftalikSesListe}`);
        embed.addField('Haftalık Chat Aktifliği',`\`Toplam =>\`  ** ${haftalikChatToplam} mesaj** \n ${haftalikChatListe}`);
